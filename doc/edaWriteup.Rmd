...
myTheme <- dark_theme_bw(base_size = 24) +
  theme(panel.grid.minor = element_blank(),
        ...
        panel.grid.major = element_line(color = "#646464", linewidth = 0.2),
...
# This function is taken directly from the package but a slight bug with the warning is fixed
Multcent <- function (dat, bi = c(1, 2), by = 3, centre = mean, centrebyBA = c(TRUE, 
    FALSE), scalebyBA = c(TRUE, FALSE)) 
{
    if (centrebyBA[1]) {
...
# Function to pull out cophanetic correlation and proportion correct between two dendograms
compareCommunity <- function(fullModelDendro,
    ...
    numberMismatched <- length(which(common_subtrees_clusters(fullModelDendro, reducedModelDendro) == 0))

    data.frame(copheneticCorrelation = copheneticCorrelation,
               proportionCorrect = proportionCorrect,
               numberMismatched = numberMismatched/nrow(reducedClustered),
               numberClusters = k,
               numberSpecies = nrow(fullClustered))
  }
...
# speciesCatchRate %>%
#   pivot_longer(c(FMWT, SS), names_to = "study", values_to = "catch") %>% 
...
Four evaluation metrics can be used to numerically measure how similar the two dendograms are: 1) proportion of clusters identified correctly, 2) number of species mismatches, and 3) cophenetic correlation. The proportion of clusters identified correctly is calculated as the proportion of subgroups of species that are clustered the same between both dendograms; the number of species mismatches enumerates the number of species that ranked differently between both dendeograms; and the cophenetic correlation is a measure of how similar the distance matrices are between both dendograms.

```{r evaluation metrics}
compareCommunity(clustered$FMWT$`5`$taxaDendro, clustered$SS$`5`$taxaDendro, k = 7) %>% 
  kbl(caption = "Evaluation metrics comparing two dendograms.") %>% 
